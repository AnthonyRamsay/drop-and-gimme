...
function updateMotivationalMessage() {
    ...
    if (count >= 0 && count <= 3) { // Displays "KEEP CLICKING" from #0 to #3
        message.innerText = 'KEEP CLICKING';
        message.style.display = 'block';
    } else if (count >= 10 && count <= 15) { // Range from #10 to #15
        ...
        message.innerText = 'KEEP GOING!!';
        message.style.display = 'block';
    } else if (count >= 100 && count <= 360) { // Range from #100 to #360
        message.innerText = 'THANKS FOR PLAYING';
        message.style.display = 'block';
    } else {
        message.style.display = 'none'; // Hide the message unless within a specific range
    }
}
...
function resetClickTimer() {
    ...
    clickTimer = setTimeout(showFailureState, 1500);
}
